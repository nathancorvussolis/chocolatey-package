...
if(($osVersion -eq [version]"6.0.6000") -or ($osVersion -eq [version]"6.0.6001")) {
  throw "$packageName needs Service Pack 2 on Windows Vista / Server 2008."
  return
}
...
if($osVersion -eq [version]"6.0.6002") {
  ...
  if(($hotfix971512 -eq $null) -AND ($hotfix971644 -eq $null)) {
    throw "$packageName needs KB971512 or KB971644 on Windows Vista / Server 2008."
    return
  }
}

if($osVersion -eq [version]"6.1.7600") {
  throw "$packageName needs Service Pack 1 on Windows 7 / Server 2008 R2."
  return
}
...
if($osVersion -eq [version]"6.3.9600") {
  ...
  if($hotfix -eq $null) {
    throw "$packageName needs KB2919355 on Windows 8.1 / Server 2012 R2."
    return
  }
}
...
Write-Warning "Restart Windows for complete installation."
